...
public class InternetOrder implements Order{
    ...
    public boolean add(Item d){//добавление нового элемента
        if (d.getName() != null) {
    ...
    public void remove(String ItemName) {//удаление элемента по названию
        boolean result;
    ...
    public int removeAll(String ItemName) {//удаляет все плзиции с заданным именем
        int a = 0;
        for (Node node = this.head.next; node != this.head; node = node.next) {
            if (node.value.equals(ItemName)) {
    ...
    public String[] itemsNames(){//вывод
    ...
    public int itemQuantity(String itemName){//хз
        int a = 0;
        for(int i = 0;i<size;i++){
    ...
    public double costTotal(){//общая стоимость заказа
        int a = 0;
        for(int i = 0; i < size;i++){
    ...
    public Node getNodeByIndex(int index) {//функциональная тема
    ...
    public Object[] names_without_equals(){//массив заказов без повторов
    ...
    public Object[] sort_by_price(){//сортировка по цене
        HashMap<Integer, String> h = new HashMap<>();
        for(int i = 0; i<size; i++)
            if(!h.containsValue(getNodeByIndex(i).value))
